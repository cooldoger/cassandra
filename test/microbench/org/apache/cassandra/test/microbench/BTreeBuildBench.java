...
public class BTreeBuildBench
{
    ...
    @Param({"treeBuilderRecycleAdd", "treeBuilderAdd"})
    String treeBuilder;
    ...
    private int treeBuilderRecycleAdd(List<Integer> data)
    {
        BTree.Builder<Integer> builder = BTree.builder(Comparator.naturalOrder());
        builder.auto(false);
        for (Integer v : data)
    ...
    {
        BTree.Builder<Integer> builder = new BTree.Builder<>();
        builder.auto(false);
        for (Integer v : data)
            builder.add(v);
        Object[] btree = builder.noRecycleBuild();
        return BTree.size(btree);
    }
    ...
    public void buildTreeTest()
    {
        ...
        switch (treeBuilder)
        {
            case "buildTree":
                size = buildTree(data);
                break;
            case "treeBuilderAddAll":
                size = treeBuilderAddAll(data);
                break;
            case "treeBuilderRecycleAdd":
                size = treeBuilderRecycleAdd(data);
                break;
            case "treeBuilderAdd":
                size = treeBuilderAdd(data);
                break;
            default:
                assert false : "unsupported operation" + treeBuilder;
        }
